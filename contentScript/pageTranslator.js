...
function backgroundTranslateHTML(translationService, sourceLanguage, targetLanguage, sourceArray2d, dontSortResults) {
  return translationService.backgroundTranslateHTML("google", sourceLanguage, targetLanguage, sourceArray2d, dontSortResults);
...
function backgroundTranslateText(translationService, sourceLanguage, targetLanguage, sourceArray) {
  return translationService.backgroundTranslateText("google", sourceLanguage, targetLanguage, sourceArray);
  return new Promise((resolve, reject) => {
    // chrome.runtime.sendMessage({
...
function backgroundTranslateSingleText(translationService, sourceLanguage, targetLanguage, source) {
  return translationService.translateSingleText(translationService, sourceLanguage, targetLanguage, source);
}
var pageTranslator = {};
...
Promise.all([]).then(function (_) {
  ...
  let currentPageTranslatorService = "bing";
  let customDictionary = sortDictionary(new Map());
  ...
  let removedNodes = [];
  let nodesToRestore = [];
  let translateDynamicallyCreatedContent = "yes";
  ...
  function enableMutatinObserver() {
    ...
    if (translateDynamicallyCreatedContent == "yes") {
  ...
  function isNoTranslateNode(node) {
    ...
    if (nodeName === "span" && node.classList.contains("mjx-chtml")) {
      return true;
    } else if (node.classList.toString().indexOf("monaco") != -1 || !hasNoMonacoParent(node)) {
      return true;
    } else if (index === -1) {
      return false;
    } else {
      ...
      if (piecesToTranslate && pageIsVisible) {
        (function () {
          ...
          if (piecesToTranslateNow.length > 0) {
            translationService.translateHTML(currentPageTranslatorService, currentSourceLanguage, currentTargetLanguage, piecesToTranslateNow.map(ptt => ptt.nodes.map(node => filterKeywordsInText(node.textContent, customDictionary, currentPageTranslatorService))), dontSortResults).then(results => {
          ...
          if (attributesToTranslateNow.length > 0) {
            translationService.translateText(currentPageTranslatorService, currentSourceLanguage, currentTargetLanguage, attributesToTranslateNow.map(ati => ati.original)).then(results => {
              if (pageLanguageState === "translated" && currentFooCount === fooCount) {
  ...
  function translatePageTitle() {
    ...
    backgroundTranslateSingleText(translationService, currentSourceLanguage, currentTargetLanguage, originalPageTitle).then(result => {
      if (result) {
  ...
  pageTranslator.translatePage = function (sourceLanguage, targetLanguage) {
    fooCount++;
    ...
    customDictionary = sortDictionary(new Map());
